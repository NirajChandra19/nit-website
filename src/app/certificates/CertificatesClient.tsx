...
import {
  ...
  FiX, FiDownload, FiShare2, FiCheckCircle, FiLock
} from "react-icons/fi";
...
const CertificateCard = memo(({ cert }: { cert: any }) => {
  ...
  const handleDownload = () => {
    ...
    link.href = `/api/certificates/download?id=${cert.id}`;
...
export default function CertificatesClient({ 
  ...
  // Compute final data instantly without loading states
  const earnedCertificates = initialCertificates.map((cert: any, index: number) => ({
    id: cert.cert_id,
  ...
  const inProgressCourses = initialEnrollments.map((e: any) => ({
